...
#include "ns3/ndnSIM/ndn-cxx/lp/pit-token.hpp"
#include "ns3/ndnSIM/ndn-cxx/lp/tags.hpp"

namespace nfd {
namespace face {

NFD_LOG_INIT(GenericLinkServiceM);

constexpr size_t CONGESTION_MARK_SIZE =
    tlv::sizeOfVarNumber(lp::tlv::CongestionMark) +  // type
    tlv::sizeOfVarNumber(sizeof(uint64_t)) +         // length
    tlv::sizeOfNonNegativeInteger(UINT64_MAX);       // value

GenericLinkServiceM::GenericLinkServiceM(
    const GenericLinkServiceM::Options& options)
    : m_options(options),
      m_fragmenter(m_options.fragmenterOptions, this),
      m_reassembler(m_options.reassemblerOptions, this),
      m_reliability(m_options.reliabilityOptions,
                    reinterpret_cast<nfd::face::GenericLinkService*>(this)),
      m_lastSeqNo(-2),
      m_nextMarkTime(time::steady_clock::TimePoint::max()),
      m_nMarkedSinceInMarkingState(0) {
    m_reassembler.beforeTimeout.connect(
        [this](auto...) { ++this->nReassemblyTimeouts; });
    m_reliability.onDroppedInterest.connect(
        [this](const auto& i) { this->notifyDroppedInterest(i); });
    nReassembling.observe(&m_reassembler);
}

void GenericLinkServiceM::setOptions(
    const GenericLinkServiceM::Options& options) {
    m_options = options;
...
void GenericLinkServiceM::requestIdlePacket(const EndpointId& endpointId) {
    // No need to request Acks to attach to this packet from LpReliability, as
    // they are already attached in sendLpPacket
    this->sendLpPacket({}, endpointId);
}

void GenericLinkServiceM::sendLpPacket(lp::Packet&& pkt,
...
void GenericLinkServiceM::doSendInterest(const Interest& interest,
                                         const EndpointId& endpointId) {
...
void GenericLinkServiceM::doSendData(const Data& data,
                                     const EndpointId& endpointId) {
...
void GenericLinkServiceM::doSendNack(const lp::Nack& nack,
                                     const EndpointId& endpointId) {
...
void GenericLinkServiceM::encodeLpFields(const ndn::PacketBase& netPkt,
                                         lp::Packet& lpPacket) {
...
void GenericLinkServiceM::sendNetPacket(lp::Packet&& pkt,
                                        const EndpointId& endpointId,
                                        bool isInterest) {
    ...
    // Only assign sequences to fragments if packet contains more than 1
    // fragment
    if (frags.size() > 1) {
    ...
    if (m_options.reliabilityOptions.isEnabled &&
        frags.front().has<lp::FragmentField>()) {
...
void GenericLinkServiceM::assignSequence(lp::Packet& pkt) {
    pkt.set<lp::SequenceField>(++m_lastSeqNo);
}

void GenericLinkServiceM::assignSequences(std::vector<lp::Packet>& pkts) {
    std::for_each(pkts.begin(), pkts.end(),
                  [this](auto& pkt) { this->assignSequence(pkt); });
}

void GenericLinkServiceM::checkCongestionLevel(lp::Packet& pkt) {
    ...
    if (sendQueueLength > 0) {
        NFD_LOG_FACE_TRACE("txqlen=" << sendQueueLength << " threshold="
                                     << m_options.defaultCongestionThreshold
                                     << " capacity="
                                     << getTransport()->getSendQueueCapacity());
    }

    // sendQueue is above target
    if (static_cast<size_t>(sendQueueLength) >
        m_options.defaultCongestionThreshold) {
        ...
        // Mark packet if sendQueue stays above target for one interval
            ...
            // Decrease the marking interval by the inverse of the square root
            // of the number of packets marked in this incident of congestion
        ...
        NFD_LOG_FACE_DEBUG(
            "Send queue length dropped below congestion threshold");
        m_nextMarkTime = time::steady_clock::TimePoint::max();
        m_nMarkedSinceInMarkingState = 0;
    }
}

void GenericLinkServiceM::doReceivePacket(const Block& packet,
    ...
    try {
        ...
        if ((pkt.has<lp::FragIndexField>() || pkt.has<lp::FragCountField>()) &&
            ...
            NFD_LOG_FACE_WARN(
                "received fragment, but reassembly disabled: DROP");
            return;
        }
        ...
        Block netPkt;
        lp::Packet firstPkt;
        std::tie(isReassembled, netPkt, firstPkt) =
            m_reassembler.receiveFragment(endpoint, pkt);
        if (isReassembled) {
...
void GenericLinkServiceM::decodeNetPacket(const Block& netPkt,
    ...
    try {
        ...
        default:
            ++this->nInNetInvalid;
            NFD_LOG_FACE_WARN("unrecognized network-layer packet TLV-TYPE "
                              << netPkt.type() << ": DROP");
            return;
        }
    } catch (const tlv::Error& e) {
...
void GenericLinkServiceM::decodeInterest(const Block& netPkt,
                                         const lp::Packet& firstPkt,
                                         const EndpointId& endpointId) {
    ...
    if (firstPkt.has<lp::HopCountTagField>()) {
        interest->setTag(make_shared<lp::HopCountTag>(
            firstPkt.get<lp::HopCountTagField>() + 1));
    }

    if (m_options.enableGeoTags && firstPkt.has<lp::GeoTagField>()) {
        interest->setTag(
            make_shared<lp::GeoTag>(firstPkt.get<lp::GeoTagField>()));
    }

    if (firstPkt.has<lp::NextHopFaceIdField>()) {
        if (m_options.allowLocalFields) {
            interest->setTag(make_shared<lp::NextHopFaceIdTag>(
                firstPkt.get<lp::NextHopFaceIdField>()));
        } else {
            NFD_LOG_FACE_WARN(
                "received NextHopFaceId, but local fields disabled: DROP");
            return;
        }
    }
    ...
    if (firstPkt.has<lp::CongestionMarkField>()) {
        interest->setTag(make_shared<lp::CongestionMarkTag>(
            firstPkt.get<lp::CongestionMarkField>()));
    }

    if (firstPkt.has<lp::NonDiscoveryField>()) {
        if (m_options.allowSelfLearning) {
            interest->setTag(make_shared<lp::NonDiscoveryTag>(
                firstPkt.get<lp::NonDiscoveryField>()));
        } else {
            NFD_LOG_FACE_WARN(
                "received NonDiscovery, but self-learning disabled: IGNORE");
        }
    }
    ...
    if (firstPkt.has<lp::PitTokenField>()) {
        interest->setTag(
            make_shared<lp::PitToken>(firstPkt.get<lp::PitTokenField>()));
    }

    this->receiveInterest(*interest, endpointId);
}

void GenericLinkServiceM::decodeData(const Block& netPkt,
                                     const lp::Packet& firstPkt,
                                     const EndpointId& endpointId) {
    ...
    if (firstPkt.has<lp::HopCountTagField>()) {
        data->setTag(make_shared<lp::HopCountTag>(
            firstPkt.get<lp::HopCountTagField>() + 1));
    }
    ...
    if (firstPkt.has<lp::CachePolicyField>()) {
        // CachePolicy is unprivileged and does not require allowLocalFields
        // option. In case of an invalid CachePolicyType,
        // get<lp::CachePolicyField> will throw, so it's unnecessary to check
        // here.
        data->setTag(make_shared<lp::CachePolicyTag>(
            firstPkt.get<lp::CachePolicyField>()));
    }
    ...
    if (firstPkt.has<lp::CongestionMarkField>()) {
        data->setTag(make_shared<lp::CongestionMarkTag>(
            firstPkt.get<lp::CongestionMarkField>()));
    }
    ...
    if (firstPkt.has<lp::PrefixAnnouncementField>()) {
        if (m_options.allowSelfLearning) {
            data->setTag(make_shared<lp::PrefixAnnouncementTag>(
                firstPkt.get<lp::PrefixAnnouncementField>()));
        } else {
            NFD_LOG_FACE_WARN(
                "received PrefixAnnouncement, but self-learning disabled: "
                "IGNORE");
        }
    }
...
void GenericLinkServiceM::decodeNack(const Block& netPkt,
                                     const lp::Packet& firstPkt,
                                     const EndpointId& endpointId) {
    BOOST_ASSERT(netPkt.type() == tlv::Interest);
    BOOST_ASSERT(firstPkt.has<lp::NackField>());
    ...
    if (firstPkt.has<lp::CongestionMarkField>()) {
        nack.setTag(make_shared<lp::CongestionMarkTag>(
            firstPkt.get<lp::CongestionMarkField>()));
    }
