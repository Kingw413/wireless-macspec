 ...
 */
class GenericLinkServiceCountersM : public virtual LinkService::Counters {
  public:
    ...
    /** \brief count of dropped partial network-layer packets due to reassembly
     * timeout
     */
    ...
    /** \brief count of network-layer packets that did not require
     * retransmission of a fragment
     */
    PacketCounter nAcknowledged;

    /** \brief count of network-layer packets that had at least one fragment
     * retransmitted, but were eventually received in full
     */
    PacketCounter nRetransmitted;

    /** \brief count of network-layer packets dropped because a fragment reached
     * the maximum number of retransmissions
     */
    PacketCounter nRetxExhausted;

    /** \brief count of outgoing LpPackets that were marked with congestion
     * marks
     */
    PacketCounter nCongestionMarked;
};

/** \brief GenericLinkService is a LinkService that implements the NDNLPv2
 * protocol \sa https://redmine.named-data.net/projects/nfd/wiki/NDNLPv2
 */
class GenericLinkServiceM FINAL_UNLESS_WITH_TESTS
    : public LinkService,
      protected virtual GenericLinkServiceCountersM {
  public:
    ...
    class Options {
      ...
      public:
        /** \brief enables encoding of IncomingFaceId, and decoding of
         * NextHopFaceId and CachePolicy
         */
         ...
         *
         *  Packets are marked if the queue size stays above THRESHOLD for at
         * least one INTERVAL.
         *
         ...
         * least one INTERVAL.
         *
         *  The default value (64 KiB) works well for a queue capacity of 200
         * KiB.
         */
         ...
         *
         *  To enable, set value of enableGeoTags option to a function that
         * generates `shared_ptr<GeoTag>`
         */
    ...
    /** \brief counters provided by GenericLinkService
     */
    using Counters = GenericLinkServiceCountersM;

    explicit GenericLinkServiceM(const Options& options = {});
     ...
     */
    void sendLpPacket(lp::Packet&& pkt, const EndpointId& endpointId);

    /** \brief send Interest
     */
    void doSendInterest(const Interest& interest, const EndpointId& endpointId)
        OVERRIDE_WITH_TESTS_ELSE_FINAL;

    /** \brief send Data
     */
    void doSendData(const Data& data, const EndpointId& endpointId)
        OVERRIDE_WITH_TESTS_ELSE_FINAL;

    /** \brief send Nack
     */
    void doSendNack(const ndn::lp::Nack& nack, const EndpointId& endpointId)
        OVERRIDE_WITH_TESTS_ELSE_FINAL;
  ...
  private:  // send path
     ...
     */
    void sendNetPacket(lp::Packet&& pkt, const EndpointId& endpointId,
                       bool isInterest);
    ...
    /** \brief if the send queue is found to be congested, add a congestion mark
     * to the packet according to CoDel \sa https://tools.ietf.org/html/rfc8289
     */
  ...
  private:  // receive path
     ...
     */
    void doReceivePacket(const Block& packet, const EndpointId& endpoint)
        OVERRIDE_WITH_TESTS_ELSE_FINAL;
     ...
     *  \param netPkt reassembled network-layer packet; TLV-TYPE must be
     * Interest \param firstPkt LpPacket of first fragment; must not have Nack
     * field \param endpointId endpoint of peer who sent the Interest
     *
    ...
    /** \brief decode incoming Interest
     *  \param netPkt reassembled network-layer packet; TLV-TYPE must be
     * Interest \param firstPkt LpPacket of first fragment; must have Nack field
...
inline const GenericLinkServiceM::Options& GenericLinkServiceM::getOptions()
    const {
    return m_options;
}

inline const GenericLinkServiceM::Counters& GenericLinkServiceM::getCounters()
    const {
    return *this;
}
