...
NS_LOG_COMPONENT_DEFINE("WIFICB-Old");

namespace ns3 {

std::string constructFaceUriOld(Ptr<NetDevice> netDevice) {
...
shared_ptr<::nfd::face::Face> WifiApStaDeviceCallbackOld(Ptr<Node> node,
    ...
    for (uint32_t i = 0; i < remotedev->GetN(); i++) {
        ...
        if(netDevice != remotedev->Get(i)){
            auto transport = make_unique<ndn::WifiNetDeviceTransportOld>(
                node, netDevice, constructFaceUriOld(netDevice),
                constructFaceUriOld(remotedev->Get(i)));
