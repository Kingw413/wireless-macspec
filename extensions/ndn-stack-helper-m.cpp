...
#include "ndn-stack-helper-m.hpp"
...
NS_LOG_COMPONENT_DEFINE("ndn.StackHelperM");

namespace ns3 {
namespace ndn {

StackHelperM::StackHelperM()
    ...
    m_csPolicies.insert(
        {"nfd::cs::lru", [] { return make_unique<nfd::cs::LruPolicy>(); }});
    m_csPolicies.insert({"nfd::cs::priority_fifo", []() {
    ...
    m_netDeviceCallbacks.push_back(std::make_pair(
        PointToPointNetDevice::GetTypeId(),
        MakeCallback(&StackHelperM::PointToPointNetDeviceCallback, this)));

    // default callback will be fired if non of others callbacks fit or did the
    // job
}

StackHelperM::~StackHelperM() {}

KeyChain& StackHelperM::getKeyChain() {
    static ::ndn::KeyChain keyChain("pib-dummy", "tpm-dummy");
    return keyChain;
}

void StackHelperM::setCustomNdnCxxClocks() {
    ::ndn::time::setCustomClocks(
        make_shared<ns3::ndn::time::CustomSteadyClock>(),
        make_shared<ns3::ndn::time::CustomSystemClock>());
}

void StackHelperM::SetDefaultRoutes(bool needSet) {
    NS_LOG_FUNCTION(this << needSet);
    m_needSetDefaultRoutes = needSet;
}

void StackHelperM::SetStackAttributes(
...
void StackHelperM::setCsSize(size_t maxSize) { m_maxCsSize = maxSize; }

void StackHelperM::setPolicy(const std::string& policy) {
...
void StackHelperM::Install(const NodeContainer& c) const {
    for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i) {
        Install(*i);
    }
}

void StackHelperM::InstallAll() const { Install(NodeContainer::GetGlobal()); }

void StackHelperM::Install(Ptr<Node> node) const {
    ...
    Simulator::ScheduleWithContext(node->GetId(), Seconds(0),
                                   &StackHelperM::doInstall, this, node);
    ProcessWarmupEvents();
}

void StackHelperM::doInstall(Ptr<Node> node) const {
    ...
    for (uint32_t index = 0; index < node->GetNDevices(); index++) {
        ...
        // This check does not make sense: LoopbackNetDevice is installed only
        // if IP stack is installed, Normally, ndnSIM works without IP stack, so
        // no reason to check if (DynamicCast<LoopbackNetDevice> (device) != 0)
...
void StackHelperM::AddFaceCreateCallback(
    TypeId netDeviceType, StackHelperM::FaceCreateCallback callback) {
    m_netDeviceCallbacks.push_back(std::make_pair(netDeviceType, callback));
}

void StackHelperM::UpdateFaceCreateCallback(TypeId netDeviceType,
...
void StackHelperM::RemoveFaceCreateCallback(TypeId netDeviceType,
                                            FaceCreateCallback callback) {
    m_netDeviceCallbacks.remove_if(
...
shared_ptr<Face> StackHelperM::DefaultNetDeviceCallback(
    Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> netDevice) const {
    ...
    auto transport = make_unique<NetDeviceTransport>(
        node, netDevice, constructFaceUri(netDevice),
        "netdev://[ff:ff:ff:ff:ff:ff]");

    auto face =
        std::make_shared<Face>(std::move(linkService), std::move(transport));
    face->setMetric(1);
...
shared_ptr<Face> StackHelperM::PointToPointNetDeviceCallback(
    Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device) const {
    ...
    Ptr<PointToPointChannel> channel =
        DynamicCast<PointToPointChannel>(netDevice->GetChannel());
...
void StackHelperM::Install(const std::string& nodeName) const {
    Ptr<Node> node = Names::Find<Node>(nodeName);
    Install(node);
}

void StackHelperM::Update(Ptr<Node> node) {
    if (node->GetObject<L3Protocol>() == 0) {
        Install(node);
        return;
    }
...
void StackHelperM::Update(const NodeContainer& c) {
    for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i) {
        Update(*i);
    }
}

void StackHelperM::Update(const std::string& nodeName) {
    Ptr<Node> node = Names::Find<Node>(nodeName);
    Update(node);
}

void StackHelperM::UpdateAll() { Update(NodeContainer::GetGlobal()); }

shared_ptr<Face> StackHelperM::createAndRegisterFace(
    Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device) const {
    ...
    if (m_needSetDefaultRoutes) {
        ...
        FibHelper::AddRoute(node, "/", face,
                            std::numeric_limits<int32_t>::max());
    }
    return face;
}

void StackHelperM::disableStrategyChoiceManager() {
    m_isStrategyChoiceManagerDisabled = true;
}

void StackHelperM::disableForwarderStatusManager() {
    m_isForwarderStatusManagerDisabled = true;
}

void StackHelperM::SetLinkDelayAsFaceMetric() {
    for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i) {
        ...
        for (auto& face : ndn->getFaceTable()) {
            auto transport =
                dynamic_cast<NetDeviceTransport*>(face.getTransport());
            if (transport == nullptr)
                continue;
            auto p2p = dynamic_cast<PointToPointChannel*>(
...
void StackHelperM::ProcessWarmupEvents() {
    Simulator::Stop(Seconds(0));
#if HAVE_NS3_VISUALIZER
    auto impl =
        DynamicCast<VisualSimulatorImpl>(Simulator::GetImplementation());
    if (impl != nullptr) {
