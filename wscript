...
OTHER_NS3_MODULES = ['antenna', 'aodv', 'bridge', 'brite', 'buildings', 'click', 'config-store', 'csma', 'csma-layout', 'dsdv', 'dsr', 'emu', 'energy', 'fd-net-device', 'flow-monitor', 'internet', 'lte', 'mesh', 'mpi', 'netanim', 'nix-vector-routing', 'olsr', 'openflow', 'point-to-point-layout', 'propagation', 'spectrum', 'stats', 'tap-bridge', 'topology-read', 'uan', 'virtual-net-device', 'wifi', 'wimax']
CUSTOM_LIB=['gsl']#'sodium','ssl','crypto'
...
def configure(conf):
    ...
    if not os.environ.__contains__('PKG_CONFIG_PATH'):
    ...
    try:
        conf.check_custom_modules(CUSTOM_LIB)
    except:
        Logs.error("Can not find libraries")
        conf.fatal("")
...
def build (bld):
    deps =  ' '.join (['ns3_'+dep for dep in MANDATORY_NS3_MODULES + OTHER_NS3_MODULES]+['custom_'+libname for libname in CUSTOM_LIB]).upper()
    common = bld.objects (
        target = "extensions",
        features = ["cxx"],
